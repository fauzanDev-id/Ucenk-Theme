...
The easiest way to swap out the Ucenk logo across the site is to replace this code with the SVG code for your company's logo or using an `<img>` tag linked to your company logo.

You should see that the Ucenk logo changes across the theme once this is updated.
...
Ucenk Theme will automatically generate a `sitemap.xml` file for you using Astro's `@astrojs/sitemap` integration. 
